      ...
      <ul>
        {messages.map((message) => (
          <li key={message.id} className={message.uid === uid ? "sent" : "received"}>
            <section>
              {/* display message text */}<div style={{display:"flex", justifyContent:"left",marginTop:"25px", alignItems:"center"}} >
              {/* display user image */}
              {message.photoURL ? (
                <img
                  ...
                  width={40}
                  height={40}
                  style={{borderRadius:"50%", marginRight:"7px", marginBottom:"10px"}}
                />
              ) : null}
